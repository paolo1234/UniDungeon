...
public class Backpack{
    ...
    public boolean isFull(){
        if(this.backpack.size()==CAPACITY)
            return true;
        return false;
    }
    ...
    //Returns false if player is trying to use other than ItemRevive if target is KO, true if succeded
    public boolean useItem(int index, Player player){
        if( (!(this.backpack.get(index) instanceof ItemRevive) && player.isKO()))
            return false;
