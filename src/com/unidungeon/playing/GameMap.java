...
public class GameMap {
    ...
    public GameMap(String name, int worldCol, int worldRow, int playerStartX, int playerStartY, int[][] levelTileNumbers, List<Decorazioni> decoration, List<Interagibili> interagibili, String soundKey){
        this.name = name;
    ...
    List<int[]> randomTriggers(){
        ...
        for (int i = 0; i < levelTileNumbers.length; i++) {
            for (int j = 0; j < levelTileNumbers[i].length; j++) {
                if (levelTileNumbers[i][j] == 1 || levelTileNumbers[i][j] == 2 || levelTileNumbers[i][j] == 8) {
