...
public class GameStateTrigger extends Interagibili {
  ...
  public GameStateTrigger(
      int worldX, int worldY, boolean solid, Rectangle boxCollider, GameState gameState) {
    this.worldX = worldX;
    this.worldY = worldY;
    this.solid = solid;
    ...
    try {
      this.image =
          ScaleImage.scaleImage(
              ImageIO.read(Objects.requireNonNull(getClass().getResource(imagePath))),
              width,
              height);
    } catch (IOException e) {
      e.printStackTrace();
  ...
  public void action() {
    ...
    if (gameState == GameState.BATTLE)
      GameMapManager.getInstance().getCurrentGameMap().getInteragibili().remove(this);
