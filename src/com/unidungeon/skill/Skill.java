...
public class Skill {
  ...
  // Execute the skill -- Skill dmg * Exectuer dmg moltiplicator * Counter role moltiplicator *
  // (Crit or NotCrit) / Receiver def moltiplicator.
  public void executeSkill(
      Entity executer, Entity receiver, Malus malus[], int malusT[], int index) {
    Random random = new Random();
    if (executer instanceof Player) ((Player) executer).decreaseSp(this.cost_sp);
    // Checks if crit hits.
    float damage =
        (this.dmg * executer.getDmg())
            * Role.counter(this.role, receiver.getRole())
            / receiver.getDef();
    // System.out.println(Role.counter(this.role,receiver.getRole()));
    // System.out.println("DMG: "+ damage);
    if (executer.hasCrit()) {
      receiver.decreaseHp(
          (int) (damage * this.CRIT_DMG)); // CRIT    40 * 1,25 /2   int 40/2 20   float 502 25
    } else {
    ...
    if (!receiver.isKO()) {
      if (this.hasMalus()) {
        ...
        if (this.malus instanceof MalusStun) malusT[index] = ((MalusStun) this.malus).getTurn();
        else malusT[index] = 3;
      }
    } else
    // If receiver is KO after the attack, remove the malus is possessed
    if (malus[index] != null) malus[index] = null;
  }

  // Returns true if the malus does hit otherwise false if the malus doesn't hit.
  public boolean hasMalus() {
    Random random = new Random();
    if (random.nextInt(100) <= this.prob_malus && prob_malus > 0) return true;
    else return false;
  }
